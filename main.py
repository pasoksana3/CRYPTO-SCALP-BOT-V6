...
def tg(s):
    if TG_TOKEN and TG_CHAT_ID:
        try:
            requests.post(f"https://api.telegram.org/bot{TG_TOKEN}/sendMessage",data={"chat_id":TG_CHAT_ID,"text":s},timeout=10).raise_for_status()
...
def can_signal(sym):
    ...
    if now-last<COOLDOWN_SECONDS:return False,int(COOLDOWN_SECONDS-(now-last))
    return True,0
...
def reverse_confirm(rows,original_side):
    ...
    for n in range(1,REVERSE_CONFIRM_5M+1):
        x=r[-n]
        if need=="SHORT" and not x[4]<x[1]:return False
        if need=="LONG" and not x[4]>x[1]:return False
    last=r[-1];prev=r[-2]
    if need=="SHORT":
...
def monitor_reverse(sym,m5):
    st=state.get(sym)
    if not isinstance(st,dict) or not st.get("side") or st.get("reverse_sent"):return
    if time.time()-float(st.get("reverse_time",0))<REVERSE_COOLDOWN_SECONDS:return
    if not reverse_confirm(m5,st["side"]):return

    price=m5[-2][4]
    side="SHORT" if st["side"]=="LONG" else "LONG"
    if side=="LONG":
        tp=price*(1+TP_PCT);sl=price*(1-SL_PCT);icon="🟢 LONG"
    else:
        tp=price*(1-TP_PCT);sl=price*(1+SL_PCT);icon="🔴 SHORT"
    ...
    tg(msg)
    st["reverse_sent"]=True;st["reverse_time"]=time.time()
    state[sym]=st;save_state(state)
    log.info("%s | REVERSE %s sent",sym,side)
...
def scan(sym):
    try:
        ...
        log.info("%s | STEP 5 | %s sweep=%s",sym,"SSL" if cand=="LONG" else"BSL",ok)
        if not ok:return
        fv=fvg(m10,cand);rt=retest(m10,cand);cf=confirm(m5,cand)
        log.info("%s | STEP 6 | FVG=%s",sym,fv);log.info("%s | STEP 7 | retest=%s",sym,rt);log.info("%s | STEP 8 | 5m confirmation=%s",sym,cf)
        ...
        price=m10[-2][4]
        if cand=="LONG":
            entry=price;tp=entry*(1+TP_PCT);sl=entry*(1-SL_PCT);icon="🟢 LONG"
        else:
            entry=price;tp=entry*(1-TP_PCT);sl=entry*(1+SL_PCT);icon="🔴 SHORT"
        msg=(f"{icon}\n{sym} Futures\nEntry: {entry:.8g}\n"
             f"TP: {tp:.8g} ({TP_PCT*100:.2f}%)\n"
             f"SL: {sl:.8g} ({SL_PCT*100:.2f}%)\n"
             f"Leverage: {LEVERAGE}x\n"
        ...
        tg(msg)
        state[sym]={"signal_time":time.time(),"side":cand,"entry":entry,"tp":tp,"sl":sl,"reverse_sent":False,"reverse_time":0}
        save_state(state)
        log.info("%s | SIGNAL %s | TP=%.2f%% | SL=%.2f%% | cooldown=%ss",sym,cand,TP_PCT*100,SL_PCT*100,COOLDOWN_SECONDS)
    except Exception as e:log.exception("%s | ERROR | %s",sym,e)

def main():
    log.info("=== CRYPTO SCALP BOT V6.4 + REVERSE ===")
    log.info("TP=%.2f%% | SL=%.2f%% | cooldown=%ss | leverage=%sx",TP_PCT*100,SL_PCT*100,COOLDOWN_SECONDS,LEVERAGE)
    while True:
        for s in SYMBOLS:scan(s)
        log.info("========== SCAN COMPLETE | sleeping %ss ==========",POLL_SECONDS)
