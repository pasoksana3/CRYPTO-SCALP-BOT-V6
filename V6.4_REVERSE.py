...
def reverse_confirm(rows,original_side):
    ...
    if need=="SHORT":
        return last[4]<prev[3] or (
            last[4]<last[1] and last[3]<prev[3]
        )

    return last[4]>prev[2] or (
        last[4]>last[1] and last[2]>prev[2]
    )

def can_signal(sym):
    now=time.time()
    last=float(state.get(sym,{}).get("signal_time",0))
    if now-last<COOLDOWN_SECONDS:
        return False,int(COOLDOWN_SECONDS-(now-last))
    return True,0

def send_reverse(sym,original_side,price):
    st=state.get(sym,{})
    now=time.time()

    if now-float(st.get("reverse_time",0))<REVERSE_COOLDOWN_SECONDS:
        return

    side="SHORT" if original_side=="LONG" else "LONG"
...
def monitor_reverse(sym,m5):
    st=state.get(sym,{})
    original=st.get("side")

    if not original or st.get("reverse_sent"):
        return

    if reverse_confirm(m5,original):
        send_reverse(sym,original,m5[-2][4])
...
def main():
    ...
    log.info(
        "TP=%.2f%% | SL=%.2f%% | cooldown=%ss | leverage=%sx",
        TP_PCT*100,SL_PCT*100,COOLDOWN_SECONDS,LEVERAGE
    )
